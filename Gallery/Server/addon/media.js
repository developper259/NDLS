...
class Media {
    ...
    static async restoreFromTrash(mediaId) {
        try {
            ...
            console.error('Erreur lors de la restauration depuis la corbeille:', error);
            throw error;
        }
    }
    ...
    static async deletePermanently(mediaId) {
        try {
            ...
            const filename = path.basename(media.file_name, path.extname(media.file_name));
            // Supprimer le fichier physique
            if (media.file_path) {
                await fs.remove(path.join(config.paths.media, media.file_name)).catch(console.error);
                await fs.remove(path.join(config.paths.thumbs, filename + ".jpg")).catch(console.error);
            }
    ...
    static async emptyTrash() {
        try {
            ...
            const trashedMedia = await db.all('SELECT * FROM media WHERE is_trashed = 1');
            ...
            // Supprimer les entrées dans album_media
            await db.run('DELETE FROM album_media WHERE media_id IN (SELECT id FROM media WHERE is_trashed = 1)');
    ...
    static async getTrashed() {
        try {
            return await db.all('SELECT * FROM media WHERE is_trashed = 1 ORDER BY trashed_at DESC');
        } catch (error) {
